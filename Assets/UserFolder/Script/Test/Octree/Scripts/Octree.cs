...
public class Octree : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        {
            if (i % 3 == 0) Gizmos.color = Color.red;
            else if (i % 3 == 1) Gizmos.color = Color.green;
    ...
    private void Update()
    {
        ...
        if (running.Count > 0)
        {
            for (int i = running.Count - 1; i >= 0; i--)
                if (!running[i].isCalculating) running.RemoveAt(i);
        }
    }
    ...
    private void GetPathAstar(object context)
    {
        ...
        try
        {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        for (int n = 0; n < current.Neigbors[i].Length; n++)
                        {
                            ...
                            if (!weights.ContainsKey(next) || newWeight < weights[next])
                            {
                                ...
                                if (fronteer.MaxSize <= fronteer.Count) fronteer.Resize(fronteer.MaxSize * 2);
            ...
            if (!particalPath) request.path.Add(request.to);
    ...
    private OctreeElement GetNeighborRec(OctreeElement startNode, OctreeElement.Dir dir)
    {
        ...
        if (localNeighborIndex >= 0) return parent.Children[localNeighborIndex];
    ...
    {
        public OctreeElement Element { get; private set; }
    ...
    {
        public static readonly Vector3[] splitDirs = {
        ...
        {
            new []{-1,4,2,-1,1,-1}, new []{-1,5,3,-1,-1,0},
            new []{-1,6,-1,0,3,-1}, new []{-1,7,-1,1,-1,2},
            new []{0,-1,6,-1,5,-1}, new []{1,-1,7,-1,-1,4},
            new []{2,-1,-1,4,7,-1}, new []{3,-1,-1,5,-1,6}

        };

        public static readonly Dir[] OppositeDirs = { 
            Dir.R, Dir.L, 
            Dir.D, Dir.U, 
            Dir.B, Dir.F 
        };
