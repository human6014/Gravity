...
public class BoidsMovementCPU : MonoBehaviour
{
    ...
    List<BoidsMovementCPU> m_Neighbours = new List<BoidsMovementCPU>();
    private IEnumerator FindNeighbourCoroutine()
    {
        Collider[] colls;
        BoidsMovementCPU neighbour;
        while (true)
        {
            m_Neighbours.Clear();
            ...
            for (int i = 0; i < colls.Length && i <= settings.maxNeighbourCount; i++)
            {
                ...
                {
                    neighbour = colls[i].GetComponent<BoidsMovementCPU>();
